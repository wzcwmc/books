...
public class BookDaoImpl implements BookDao {
	...
	public List<Book> getBookBytype(String bookType,int pageIndex, int pageSize) {
		...
		sb.append(" from(select b.*,rownum as rowno ");
		sb.append(" from book b where booktype like '%'||?||'%' ) bb ");
	...
	public Book getBookById(int bookId) {
		...
		list=getBookBySQL(sb.toString(), bookId);
		if (list.size()>0) {
			return list.get(0);
		}
		return null;
	...
	public Book getBookByIsbn(String Isbn) {
		...
		list=getBookBySQL(sb.toString(), Isbn);
		if (list.size()>0) {
			return list.get(0);
		}
		return null;
	...
	public int getCountsBybookType(String bookType) {
		int count=0;
		String sql=" select count(0) as rowno from book where booktype like '%'||?||'%' ";
		conn=dbu.getConn();
		try {
			psmt= conn.prepareStatement(sql);
			psmt.setString(1, bookType);
			rs=psmt.executeQuery();
			if(rs.next())
			{
				count=rs.getInt("rowno");
	...
	public int getCountsByString(String str) {
		int count=0;
		String sql=" select count(0) as rowno from book where bookname "
				+ " like '%'||?||'%' or booktype like '%'||?||'%' ";
		conn=dbu.getConn();
		try {
			psmt= conn.prepareStatement(sql);
